// AUTH NAVIGATION SCREEN

export const SIGN_IN_SCREEN = "SignIn";
export const FORGOT_PASSWORD_SCREEN = "ForgotPassword";
export const SIGN_UP_SCREEN = "SignUp";

// Navigator
export const BOTTOM_TAB_NAV = "BottomTabNavigator";
export const SETTINGS_NAVIGATOR = "SettingsNavigator";
export const DRAWER_NAVIGATOR = "DrawerNavigator";

// HOME NAVIGATION SCREENS

export const HOME_SCREEN = "HomeScreen";
export const BROWSE_SCREEN = "BrowseScreen";
export const SHOP_SCREEN = "ShopScreen";
export const SETTINGS_SCREEN = "SettingsScreen";
export const SETTINGS_DETAILS = "SettingDetails";
export const REST_DETAILS = "RestaurantDetails";
export const DEALS_SCREEN = "Deals";
export const ORDER_DETAILS = "OrderDetails";
export const TELEGRAPH_DETAILS = "TelegraphDetails";
export const TRACK_ORDER_MAP = "TrackOrderMap";
export const STORE_DETAILS = "StoreDetails";
export const SAFE_DETAILS = "SafeDeatils";
export const CATEGORY_DETAILS = "CategoriesDetails";
export const REGISTER_DETAILS = "RegisterScreen";
export const SEARCH_DETAILS = "SearchScreen";
